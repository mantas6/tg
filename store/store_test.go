...
// TestMigrateAddsGapColumn covers the v5 upgrade of a database written before
// gap entries existed: the column is added in place, every row that predates it
// reads back as an ordinary tracked entry (the whole point of the 0 default),
// and the migrated store round-trips the flag.
func TestMigrateAddsGapColumn(t *testing.T) {
	t.Parallel()
	path := filepath.Join(t.TempDir(), "tg.db")

	// A pre-v5 entries table: everything up to seq, no gap column.
	raw, err := sql.Open("sqlite", path)
	if err != nil {
		t.Fatalf("open raw: %v", err)
	}
...
CREATE TABLE entries (
  ...
  seq INTEGER NOT NULL DEFAULT 0, updated_at TEXT NOT NULL, synced_at TEXT,
  dirty INTEGER NOT NULL DEFAULT 1, deleted INTEGER NOT NULL DEFAULT 0);
CREATE TABLE meta (key TEXT PRIMARY KEY, value TEXT);
INSERT INTO entries (workspace_id, description, start, stop, duration, seq, updated_at)
VALUES (1, 'old', '2026-01-02T09:00:00Z', '2026-01-02T10:00:00Z', 3600, 1,
        '2026-01-02T10:00:00Z');`); err != nil {
		t.Fatalf("seed old schema: %v", err)
	}
	raw.Close()
	...
	has, err := s.hasColumn(ctx, "entries", "gap")
	if err != nil {
		t.Fatalf("hasColumn: %v", err)
	}
	if !has {
		t.Fatal("entries.gap missing after migrate")
	}
	if v, ok := mustMeta(t, s, MetaSchemaVersion); !ok || v != schemaVersion {
		t.Errorf("schema_version = %q ok=%v, want %q", v, ok, schemaVersion)
	}
	...
	got, err := s.EntriesBetween(ctx, at, at.AddDate(0, 0, 1))
	if err != nil {
		t.Fatalf("between: %v", err)
	}
	if len(got) != 1 {
		t.Fatalf("entries = %d, want the pre-existing one", len(got))
	}
	if got[0].Gap {
		t.Error("a row written before the column existed must read back as a tracked entry, not a gap")
	}

	// And the migrated table stores the flag.
	gapStart := at.Add(4 * time.Hour)
	mustCreate(t, s, Entry{
		WorkspaceID: 1, Start: gapStart, Stop: ptrTime(gapStart.Add(time.Hour)),
		Duration: 3600, Gap: true, UpdatedAt: gapStart,
	})
	got, err = s.EntriesBetween(ctx, at, at.AddDate(0, 0, 1))
	if err != nil {
		t.Fatalf("between: %v", err)
	}
	if len(got) != 2 || !got[1].Gap {
		t.Errorf("entries = %+v, want the second one to be a gap", got)
	}
}

// TestGapEntryIsAnOrdinaryRow pins what a gap entry shares with a tracked one:
// it is numbered by its day, it occupies its span as far as the overlap guard is
// concerned, it can be "the last entry" a relative span continues from, and it
// is addressable by its number. Only the sync boundary treats it differently
// (see TestDirtyEntriesExcludeGaps).
func TestGapEntryIsAnOrdinaryRow(t *testing.T) {
	t.Parallel()
	s := openTest(t)
	nine := time.Date(2026, 1, 2, 9, 0, 0, 0, time.UTC)
	ten := nine.Add(time.Hour)
	eleven := nine.Add(2 * time.Hour)

	mustCreate(t, s, Entry{
		WorkspaceID: 1, Start: nine, Stop: ptrTime(ten), Duration: 3600,
		UpdatedAt: ten, Dirty: true,
	})
	gapID := mustCreate(t, s, Entry{
		WorkspaceID: 1, Start: ten, Stop: ptrTime(eleven), Duration: 3600,
		Gap: true, UpdatedAt: ten,
	})

	// Numbering: the gap took the day's next number rather than being skipped.
	got, err := s.EntryByNum(ctx, 2, nine)
	if err != nil {
		t.Fatalf("EntryByNum(2): %v", err)
	}
	if got.ID != gapID || !got.Gap {
		t.Errorf("entry 2 = id %d gap=%v, want the gap (id %d)", got.ID, got.Gap, gapID)
	}

	// Overlap: the gap's span is taken.
	clashes, err := s.FindOverlapping(ctx, ten.Add(30*time.Minute), eleven.Add(30*time.Minute))
	if err != nil {
		t.Fatalf("FindOverlapping: %v", err)
	}
	if len(clashes) != 1 || clashes[0].ID != gapID {
		t.Errorf("overlaps = %+v, want just the gap", clashes)
	}

	// "The last entry" is the gap, so a bare duration continues after it.
	last, err := s.LastEntry(ctx, eleven.Add(time.Hour))
	if err != nil {
		t.Fatalf("LastEntry: %v", err)
	}
	if last == nil || last.ID != gapID || !last.Gap {
		t.Errorf("LastEntry = %+v, want the gap (id %d)", last, gapID)
	}
}

// TestDirtyEntriesExcludeGaps pins the sync boundary at the query that defines
// the push queue: a gap never enters it, however dirty the row looks, while the
// tracked entry next to it still does. It is what keeps `tg mod`/`tg del` on a
// gap from ever reaching Toggl.
func TestDirtyEntriesExcludeGaps(t *testing.T) {
	t.Parallel()
	s := openTest(t)
	at := time.Date(2026, 1, 2, 9, 0, 0, 0, time.UTC)
	trackedID := mustCreate(t, s, Entry{
		WorkspaceID: 1, Start: at, Stop: ptrTime(at.Add(time.Hour)),
		Duration: 3600, UpdatedAt: at, Dirty: true,
	})
	// Dirty on purpose: an edit (store.UpdateEntry) or a delete marks a gap
	// dirty like anything else, and that must still not queue it.
	gapID := mustCreate(t, s, Entry{
		WorkspaceID: 1, Start: at.Add(2 * time.Hour), Stop: ptrTime(at.Add(3 * time.Hour)),
		Duration: 3600, Gap: true, UpdatedAt: at, Dirty: true,
	})

	dirty := mustDirty(t, s)
	if len(dirty) != 1 || dirty[0].ID != trackedID {
		t.Fatalf("dirty = %+v, want only the tracked entry (id %d)", dirty, trackedID)
	}

	// Soft-deleting the gap does not queue it either, and the row survives so
	// its number stays retired.
	if err := s.SoftDeleteEntry(ctx, gapID, at.Add(4*time.Hour)); err != nil {
		t.Fatalf("soft delete gap: %v", err)
	}
	if dirty := mustDirty(t, s); len(dirty) != 1 || dirty[0].ID != trackedID {
		t.Errorf("dirty after deleting the gap = %+v, want only the tracked entry", dirty)
	}
	if _, err := s.EntryByNum(ctx, 2, at); !errors.Is(err, ErrNoEntryNum) {
		t.Errorf("EntryByNum(2) after delete = %v, want ErrNoEntryNum", err)
	}
	next := mustCreate(t, s, Entry{
		WorkspaceID: 1, Start: at.Add(5 * time.Hour), Stop: ptrTime(at.Add(6 * time.Hour)),
		Duration: 3600, UpdatedAt: at,
	})
	got, err := s.EntryByNum(ctx, 3, at)
	if err != nil {
		t.Fatalf("EntryByNum(3): %v", err)
	}
	if got.ID != next {
		t.Errorf("entry 3 = id %d, want %d: a deleted gap keeps its number", got.ID, next)
	}
}

// TestUpdateEntryKeepsGapFlag pins that what an entry IS never changes under an
// edit: retiming and re-describing a gap leaves it a gap (and a tracked entry a
// tracked one), since UpdateEntry writes neither the flag nor anything that
// implies it.
func TestUpdateEntryKeepsGapFlag(t *testing.T) {
	t.Parallel()
	s := openTest(t)
	...
	id := mustCreate(t, s, Entry{
		...
		Duration: 3600, Gap: true, UpdatedAt: start,
	})

	stop := start.Add(90 * time.Minute)
	if err := s.UpdateEntry(ctx, Entry{
		ID: id, Description: "lunch", Start: start, Stop: &stop,
		Duration: 5400, UpdatedAt: now,
	}, now, time.UTC); err != nil {
		t.Fatalf("update: %v", err)
	}
	got, err := s.EntryByNum(ctx, 1, start)
	if err != nil {
		t.Fatalf("EntryByNum(1): %v", err)
	}
	if !got.Gap {
		t.Error("the edited entry stopped being a gap")
	}
	if got.Description != "lunch" || got.Duration != 5400 {
		t.Errorf("entry = (%q, %ds), want (lunch, 5400s)", got.Description, got.Duration)
	}
}
