...
func runDaily(ctx context.Context, args []string) error {
	...
	// --no-today and -n are aliases bound to the same variable: they drop
	// today's row from the listing (and from the footer's totals), which is
	// what makes `tg daily -n` report only days that are already over.
	var noToday bool
...
func printUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "timesign: absolute 9-:30, 10-11, 10:30-11:15 (today); `@` in a range is")
	fmt.Fprintln(w, "      now floored to the last 5m mark, and `@:30` (no dash) is from")
	fmt.Fprintln(w, "      now until :30. relative +:20, +1, +1:20 (that long, ending at")
	fmt.Fprintln(w, "      the last 5m mark), negative -:20, -1:20 (that much LESS; `mod`")
	fmt.Fprintln(w, "      only), or a bare duration 1:30, :45 (that long, starting where")
	fmt.Fprintln(w, "      the last entry ended; `add` only). Full spec: docs/timesig.md")
	...
	fmt.Fprintln(w, "      a relative or `@` timesign has no `now` there, so `add` refuses")
	fmt.Fprintln(w, "      one (`mod` still takes +30/-30, which only move an end).")
